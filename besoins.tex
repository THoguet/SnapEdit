...
{Transférer la liste des algorithmes implementés}
{
  ...
  \begin{description}
    ...
    \item[Name:] Le point d'accès du paramètre \verb!/images?algorithm=*algo*\&<name>=*value*! (type \verb!string!)
    \item[displayName:] Le nom d'affichage du paramètre (type \verb!string!)
    \item[options:] Le nom et la valeur associés à l'option de l'input (type \verb!string!)
  \end{description}
  \end{description}
}
{}
...
{Supprimer un élement d'une image}
{
  ...
  \item[top :] la partie supprimée est remplie de façon similaire à ce qu'il y a en haut.
  \item[bottom :] la partie supprimée est remplie de façon similaire à ce qu'il y a en bas.  
  \end{description}
  \end{description}
}
{}
...
{
  Le front doit pouvoir afficher les algorithmes donné par le backend (/algorithms)\verb!/images!.
  voir \refBesoin{backend:listAlgo}
}
{}
{}
...
{Générer une image avec stableDiffusion}
{
  ...
  \item[Netative prompt:] Une zone de texte pour le prompt negatif de l'image
  \item[Hauteur:] Un selecteur pour la hauteur de l'image
  \item[Largeur:] Un selecteur pour la largeur de l'image
  \item[Steps:] Le nombre de steps lors de la generation
...
{
  Avoir un affichage claire de la progression d'un traitement d'image / de la generation d'une image ou d'un upload
}
{}
{}
